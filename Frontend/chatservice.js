window.addEventListener("load", (event) => {
  ...
  (async function logJSONData() {
    response = await fetch("http://192.168.0.120:8090/api/v1/chat/getAll");
  ...
  async function logJSONDataAfterQuestion() {
    let nextResponse = await fetch(
      "http://192.168.0.120:8090/api/v1/chat/getAll"
    );
  ...
  const getanswer = async (question) => {
    let answer = "";
    try {
      const rawData = await fetch("http://192.168.0.120:8090/api/v1/chatHpt", {
        headers: {
